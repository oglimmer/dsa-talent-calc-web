...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "dsa-talent-calcnet"; then
			verbosePrint "Starting docker network dsa-talent-calcnet on 10.100.86.0/24"
			docker network create -d bridge --subnet 10.100.86.0/24 --gateway 10.100.86.1 "dsa-talent-calcnet"
		else
			verbosePrint "Docker network dsa-talent-calcnet already running"
		fi
	fi
}
...
Url: http://localhost:8080/dsa-talent-calc

Details for components:
dsatalentcalc {Source:\"mvn\", Default-Type:\"local\"}
  -t dsatalentcalc:local #build local and respect -j
  -t dsatalentcalc:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
dsatalentcalcweb {Source:\"mvn\", Default-Type:\"local\"}
  -t dsatalentcalcweb:local #build local and respect -j
...
if [ -z "$1" ]; then
	...
	declare START_DSATALENTCALC=YES

	declare START_DSATALENTCALCWEB=YES

	declare START_TOMCAT=YES

else
	ALL_COMPONENTS=(DSATALENTCALC DSATALENTCALCWEB TOMCAT)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="06b41e0388dcf7cf0ce530fec81db703"
...
verbosePrint "DEFAULT: TYPE_SOURCE_DSATALENTCALC=local"
TYPE_SOURCE_DSATALENTCALC=local

verbosePrint "DEFAULT: TYPE_SOURCE_DSATALENTCALCWEB=local"
TYPE_SOURCE_DSATALENTCALCWEB=local
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    echo "...then browse to http://localhost:8080/dsa-talent-calc"
		  SHELL
		end
	EOF
	vagrant up
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# MvnPlugin // dsatalentcalc
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // dsatalentcalc"

if [ "$START_DSATALENTCALC" = "YES" ]; then

	#---------------------
	# START - Plugin-PrepareComp

	mkdir -p localrun/dsatalentcalc

	OPWD="$(pwd)"
	cd "localrun/dsatalentcalc"

	IFS=',' read -r -a array <<<"$TYPE_SOURCE"
	for typeSourceElement in "${array[@]}"; do
		IFS=: read comp type pathOrVersion <<<"$typeSourceElement"

		if [ "$comp" == "dsatalentcalc" ]; then
			TYPE_SOURCE_DSATALENTCALC=$type
			if [ "$TYPE_SOURCE_DSATALENTCALC" == "local" ]; then
				TYPE_SOURCE_DSATALENTCALC_PATH=$pathOrVersion
			else
				TYPE_SOURCE_DSATALENTCALC_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_DSATALENTCALC" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_DSATALENTCALC_VERSION" ]; then
			TYPE_SOURCE_DSATALENTCALC_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_DSATALENTCALC = $TYPE_SOURCE_DSATALENTCALC // TYPE_SOURCE_DSATALENTCALC_PATH = $TYPE_SOURCE_DSATALENTCALC_PATH // TYPE_SOURCE_DSATALENTCALC_VERSION = $TYPE_SOURCE_DSATALENTCALC_VERSION"
	...
	if [ ! -d ".git" ]; then
		git clone --single-branch "https://github.com/oglimmer/dsa-talent-calc.git" .
	else
		git pull
	fi
	...
	if [ "$TYPE_SOURCE_DSATALENTCALC" == "local" ]; then
		f_build() {
			verbosePrint "pwd=$(pwd)\nmvn $MVN_CLEAN $MVN_OPTS install"
	...
	if [ "$TYPE_SOURCE_DSATALENTCALC" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=dsatalentcalc --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_DSATALENTCALC_VERSION mvn $MVN_CLEAN $MVN_OPTS install"
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# MvnPlugin // dsatalentcalcweb
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // dsatalentcalcweb"

if [ "$START_DSATALENTCALCWEB" = "YES" ]; then
	...
	for typeSourceElement in "${array[@]}"; do
		...
		if [ "$comp" == "dsatalentcalcweb" ]; then
			TYPE_SOURCE_DSATALENTCALCWEB=$type
			if [ "$TYPE_SOURCE_DSATALENTCALCWEB" == "local" ]; then
				TYPE_SOURCE_DSATALENTCALCWEB_PATH=$pathOrVersion
			else
				TYPE_SOURCE_DSATALENTCALCWEB_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_DSATALENTCALCWEB" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_DSATALENTCALCWEB_VERSION" ]; then
			TYPE_SOURCE_DSATALENTCALCWEB_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_DSATALENTCALCWEB = $TYPE_SOURCE_DSATALENTCALCWEB // TYPE_SOURCE_DSATALENTCALCWEB_PATH = $TYPE_SOURCE_DSATALENTCALCWEB_PATH // TYPE_SOURCE_DSATALENTCALCWEB_VERSION = $TYPE_SOURCE_DSATALENTCALCWEB_VERSION"
	...
	if [ "$TYPE_SOURCE_DSATALENTCALCWEB" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=dsatalentcalcweb --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_DSATALENTCALCWEB_VERSION mvn $MVN_CLEAN $MVN_OPTS package"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	f_deploy() {
		cp target/dsa-talent-calc##001.war $targetPath
	}
	f_deploy
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "download" ]; then
		if [ ! -f ".tomcatPid" ]; then
			...
			export JAVA_OPTS="$JAVA_OPTS -DJAVA_OPTS=-XX:+UnlockExperimentalVMOptions -XX:+UseCGroupMemoryLimitForHeap"
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 -m 70M --net=dsa-talent-calcnet --name=tomcat $ADD_HOST_INTERNAL  -e JAVA_OPTS="-XX:+UnlockExperimentalVMOptions -XX:+UseCGroupMemoryLimitForHeap" -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps oglimmer/adoptopenjdk-tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/dsa-talent-calc"
